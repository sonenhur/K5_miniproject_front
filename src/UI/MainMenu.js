...
export default function MainMenu() {
  ...
  const scrollLeft = () => {
    if (slideRef.current) {
      ...
      slideRef.current.scrollLeft -= slideWidth;
    }
  };
  ...
  const renderPage = () => {
    switch (currentPage) {
      case "Home":
        return (
          <div className="flex-col w-4/5 bg-black h-svh">
            <div
              className="tracking-tight ml-5 mr-5 h-1/2 border-solid border-2 border-black"
              style={{
            ...
            <div className="tracking-tight h-1/2">
              ...
              <div className="flex justify-center items-center">
                <button
                  onClick={scrollLeft}
                  className="font-semibold text-lg tracking-tight text-white mt-2 mr-1 hover:text-red-400"
                >
                  ◀️
                </button>
                <button
                  onClick={scrollRight}
                  className="font-semibold text-lg tracking-tight text-white mt-2 ml-1 hover:text-red-400"
                >
                  ▶️
                </button>
              </div>
      ...
      case "Help":
        return (
          <div className="flex justify-center items-center bg-black w-4/5">
            <svg
              className="animate-spin h-20 w-20 mr-5 text-white"
    ...
    <div>
      <nav className="flex flex-wrap justify-between items-center bg-black">
        <button
          ...
          className="flex items-center text-red-400 w-1/5 m-5"
        >
        ...
        <div className="block flex-grow lg:flex lg:items-center lg:w-auto justify-end w-4/5 m-5">
          <div className="mr-3">
            <svg
              className="fill-white h-7 w-7 mr-3"
              viewBox="0 0 448 512"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Profile</title>
      ...
      <div className="flex h-full">
        <div className="flex-col bg-zinc-800 w-1/5 p-5">
          ...
          <button
            type="button"
            onClick={() => changePage("Movie")}
            className="flex m-5"
          >
            <svg
              className="fill-white h-7 w-7 hover:fill-red-400"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 512 512"
            >
            ...
            <div className="font-semibold text-lg tracking-tight ml-3 text-white hover:text-red-400">
              영화
              <br />
            </div>
          </button>
          <Link to="/movie/0" className="flex m-5 ml-12">
            <div className="font-medium text-lg tracking-tight ml-3 text-white hover:text-red-400">
              웡카
              <br />
            </div>
          </Link>
      ...
      <footer className="flex p-5 h-36 bg-black">
        <div>
          <ul className="font-medium text-lg tracking-tight text-zinc-400 mb-5">
            StellaNeX
            <br />
          </ul>

          <ul className="font-normal text-sm tracking-tight text-zinc-400">
            전화번호 : 000-0000-0000
            <br />
          </ul>
          <ul className="font-normal text-sm tracking-tight text-zinc-400">
